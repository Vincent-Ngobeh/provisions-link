...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"   Total BuyingGroups in DB: {BuyingGroup.objects.count()}")
        ...
        self.stdout.write(f"   Vendor: {group.product.vendor.business_name} (ID: {group.product.vendor.id})")
        ...
        self.stdout.write(f"   Progress: {group.current_quantity}/{group.target_quantity}")
        ...
        for i, commitment in enumerate(commitments, 1):
            ...
            self.stdout.write(f"    Buyer: {commitment.buyer.username} (ID: {commitment.buyer.id})")
            ...
            self.stdout.write(f"    Payment Intent: {commitment.stripe_payment_intent_id or 'None'}")
            self.stdout.write(f"    Buyer Location: {commitment.buyer_location}")
            self.stdout.write(f"    Buyer Postcode: {commitment.buyer_postcode}")
        ...
        self.stdout.write("="*80)
        self.stdout.write("📞 Calling GroupBuyingService.process_expired_groups()...")
        ...
        self.stdout.write(f"   total_processed: {stats.get('total_processed', 0)}")
        ...
        for commitment in commitments:
            self.stdout.write(f"   - Commitment {commitment.id}: status={commitment.status}")

        # Check for created orders
        self.stdout.write(f"\n   Checking for orders created from this group...")
        orders = Order.objects.filter(group=group)
        self.stdout.write(f"   Orders created for this group: {orders.count()}")
        ...
        if will_succeed and stats.get('successful', 0) > 0:
            ...
            for commitment in confirmed_commitments:
                ...
                self.stdout.write(f"      Status: {commitment.status}")
                if commitment.order:
                    self.stdout.write(f"      ✅ Order created: {commitment.order.reference_number}")
                else:
            ...
            if test_commitment:
                self.stdout.write(f"📝 Testing with Commitment {test_commitment.id}:")
                self.stdout.write(f"   Status: {test_commitment.status}")
                self.stdout.write(f"   Buyer: {test_commitment.buyer.username} (ID: {test_commitment.buyer.id})")
                self.stdout.write(f"   Quantity: {test_commitment.quantity}")
                self.stdout.write(f"   Payment Intent: {test_commitment.stripe_payment_intent_id or 'None'}")
                ...
                if test_commitment.status == 'cancelled':
                    self.stdout.write("ℹ️  Commitment already processed (status: cancelled)")
        ...
        # Verify restoration
        group.refresh_from_db()
        self.stdout.write(self.style.SUCCESS(f"   ✅ Verified group expires_at: {group.expires_at}"))
        self.stdout.write(self.style.SUCCESS(f"   ✅ Verified group status: {group.status}"))

        # Summary
        self.stdout.write("\n" + "="*80)
        if will_succeed and stats.get('successful', 0) > 0:
            self.stdout.write(self.style.SUCCESS("✅ VERBOSE TEST COMPLETE - GROUP SUCCEEDED"))
        elif not will_succeed and stats.get('failed', 0) > 0:
            self.stdout.write(self.style.SUCCESS("✅ VERBOSE TEST COMPLETE - GROUP FAILED AS EXPECTED"))
        else:
            self.stdout.write(self.style.WARNING("⚠️  VERBOSE TEST COMPLETE - UNEXPECTED RESULT"))
        self.stdout.write("="*80 + "\n")
